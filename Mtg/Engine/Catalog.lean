...
/-- A legendary creature (CR 205.4 / 704.5j). Same arguments as `creature`. -/
def legendaryCreature := creature (legendary := true)
...
def instant := spellCard .instant

/-- A sorcery, optionally with a one-shot effect or modal modes. -/
def sorcery := spellCard .sorcery
...
/-- A legendary land. Same arguments as `land`. -/
def legendaryLand := land (legendary := true)
