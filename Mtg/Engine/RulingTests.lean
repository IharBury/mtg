...
def untargetedAmass : Game := started.applyEffect ⟨0⟩ (Effect.ofSpell (.amassGoblins 1)) #[]
...
def knotsIllegal : Game :=
  afterDraw.applyEffect ⟨0⟩ (Effect.ofSpell (.tapScryDraw 1 1)) #[.player ⟨1⟩]

def knotsAlreadyTapped : Game :=
  let g := addPermanent afterDraw grizzlyBears ⟨0⟩ ⟨0⟩
  let bears := namedPermanent g "Grizzly Bears"
  let g := g.setObject { bears with status := { bears.status with tapped := true } }
  g.applyEffect ⟨0⟩ (Effect.ofSpell (.tapScryDraw 1 1)) #[.permanent (namedPermanent g "Grizzly Bears").id]
...
def settleExilesAttackers : Game :=
  ...
  let g := g.mapObjectStatus (g.object! tok.id)
    (fun s => { s with attacking := true })
  g.applyEffect ⟨0⟩ (Effect.ofSpell .exileAttackersSearchBasics) #[.player ⟨1⟩]
...
def protectionFromEverythingOk : Bool :=
  let ts := withProtection.legalTargets ⟨0⟩ (Effect.ofSpell (.dealDamage 3))
...
def dwarvenWarriorsPowerRaisedStillUnblockableOk : Bool :=
  ...
  let g := addPermanent g grizzlyBears ⟨0⟩ ⟨0⟩
  let bear := namedPermanent g "Grizzly Bears"
  let g := g.applyAbilityEffect ⟨0⟩ (Effect.ofAbility (.targetCantBeBlockedPowerAtMost 2))
...
def eaglesRescueIllegalStaysOk : Bool :=
  ...
  let g := g.applyAbilityEffect ⟨0⟩ (Effect.ofAbility (.returnFromGyAttachPowerAtMost 1))
...
def uneasyPartingsOwnerChoosesOk : Bool :=
  let g := addPermanent afterDraw grizzlyBears ⟨1⟩ ⟨1⟩
  let bear := namedPermanent g "Grizzly Bears"
  let g := g.applyEffect ⟨0⟩ (Effect.ofSpell .putOnTopOrBottom)
    #[Target.permanent bear.id]
