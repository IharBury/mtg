...
- **Value** — `nat`, `int`, `x`, `greatestManaValue`, `greatestToughness`,
  `greatestPower`, `count`, `power`, `toughness`.
...
- **`setPowerToughness`** (14 cards) — Set base P/T to literal values (`setPower` and `setToughness` take a `Value`; the compiler pairs only a lands-you-control count. `setBasePower` and `setBaseToughness` take a `Value`; the compiler pairs only this source's power and toughness)
...
- `ContinuousEffect.setPowerToughness` — Set base P/T to literal values (`setPower` and `setToughness` take a `Value`; the compiler pairs only a lands-you-control count. `setBasePower` and `setBaseToughness` take a `Value`; the compiler pairs only this source's power and toughness)
