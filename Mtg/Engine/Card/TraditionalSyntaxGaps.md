...
`ContinuousEffect.addPower` of `Value.count` and `addToughness` of
`Value.product` (`Value.count` times zero) compile other-subtype +1/+0 for
...
- **`addPower` / `addToughness`** (8 cards) — Pump / set PT from a count other than `setPower` and `setToughness` of `Value.count` of lands you control. `addPower` and `addToughness` take a `Value`; the compiler pairs only `Value.count` for +1 and `Value.product` of that count with zero for other-subtype +1/+0 per artifact token
...
- `ContinuousEffect.addPower` / `addToughness` — Pump / set PT from a count other than `setPower` and `setToughness` of `Value.count` of lands you control. `addPower` and `addToughness` take a `Value`; the compiler pairs only `Value.count` for +1 and `Value.product` of that count with zero for other-subtype +1/+0 per artifact token
- `Selector.countOf` — Numeric value derived from a count or characteristic
