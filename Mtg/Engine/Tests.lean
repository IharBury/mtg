...
#guard
  ...
  | .error msg => mentions msg "2 or more creatures"
  | .ok _ => false
...
/-- Crude Bent Blade: ETB opponent sacrifices; equip +2/+1. -/
def bladeReady : Game :=
  let g := addPermanent afterDraw ragingGoblin ⟨0⟩ ⟨0⟩
  let g := addPermanent g grizzlyBears ⟨1⟩ ⟨1⟩
  let g := readyMain (emptyHand g ⟨0⟩)
...
def bladeEntered : Game :=
  let g := mustApply bladeReady ⟨0⟩
    (.cast (handCardNamed bladeReady ⟨0⟩ "Crude Bent Blade").id)
  passBoth (mustApply g ⟨0⟩ .pay)

#guard bladeEntered.pending == .chooseTargets ⟨0⟩
#guard (bladeEntered.object! bladeEntered.stack.back!.objectId).triggeredAbility ==
  some .onEnterTargetOpponentSacrifices
#guard
  match Agent.choose bladeEntered ⟨0⟩ with
  | some (.target (Target.player q)) => q == ⟨1⟩
  | _ => false

def bladeOppSacrificing : Game :=
  let g := mustApply bladeEntered ⟨0⟩ (.target (Target.player ⟨1⟩))
  passBoth g

#guard
  match bladeOppSacrificing.pending with
  | .chooseSacrificeCreature ⟨1⟩ _ _ => true
  | _ => false

def bladeAfterSac : Game :=
  mustApply bladeOppSacrificing ⟨1⟩
    (.sacrifice (namedPermanent bladeOppSacrificing "Grizzly Bears").id)

#guard !(bladeAfterSac.battlefield.any (fun o => o.name == "Grizzly Bears"))
#guard bladeAfterSac.log.any (fun s => mentions s "Nissa sacrifices Grizzly Bears")

def bladeEquipped : Game :=
  let g := withBlackMana bladeAfterSac ⟨0⟩ 2
  let blade := namedPermanent g "Crude Bent Blade"
  let g := mustApply g ⟨0⟩ (.activate blade.id 0)
  let g := mustApply g ⟨0⟩
    (.target (Target.permanent (namedPermanent g "Raging Goblin").id))
  passBoth (mustApply g ⟨0⟩ .pay)

#guard (namedPermanent bladeEquipped "Crude Bent Blade").attachedTo ==
  some (namedPermanent bladeEquipped "Raging Goblin").id
#guard bladeEquipped.power (namedPermanent bladeEquipped "Raging Goblin") == 3
#guard bladeEquipped.toughness (namedPermanent bladeEquipped "Raging Goblin") == 2
